...
const customSearchEngineCache = new LRUCache<string, Cached>({ max: 3000 })
...
metrics.trackCache('customSearchEngine', customSearchEngineCache)
...
export default new Service<Clients, RecorderState, CustomContext>({
  clients: {
    ...
    options: {
      ...
      customSearchEngine: {
        concurrency: 10,
        memoryCache: customSearchEngineCache,
        timeout: SIX_SECONDS_MS,
      },
      vbase: {
        concurrency: 2,
